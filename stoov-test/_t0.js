        ...
        // ========== MQTT 配置 ==========
        const MQTT_URL = "wss://r61d1d77.ala.cn-shenzhen.emqxsl.cn:8084/mqtt";
        const MQTT_OPTIONS = {
            username: "heatingBed",
            password: "cos8mos7",
        ...
        function mqttConnect() {
            try {
                ...
                mqttClient.on("close", () => {
                    if (isMqttConnected) {
                        bleAddLog("info", "MQTT 连接已断开");
                    }
                    isMqttConnected = false;
                });

                mqttClient.on("reconnect", () => {
                    bleAddLog("info", "MQTT 正在重连...");
                });

                mqttClient.on("offline", () => {
                    bleAddLog("info", "MQTT 离线");
                });
            } catch (error) {
        ...
        function updateHardwareUptimeDisplay(seconds) {
            ...
            document.getElementById('hardwareUptimeDisplay').textContent = `${h}:${m}:${s}`;
        }
        ...
        const TEST_DEFS = {
            external: [
                ...
                { id: 'ext-time', label: '时间调节功能', result: 'wait' },
                { id: 'ext-body', label: '身体温度调节功能', result: 'wait' },
                { id: 'ext-foot', label: '脚温度调节功能', result: 'wait' },
            ],
            internal: [
                { id: 'int-devid', label: '检测设备ID', result: 'wait' },
                { id: 'int-fault', label: '故障代码', result: 'wait' },
                { id: 'int-bsensor', label: '身体温度传感器', result: 'wait' },
                { id: 'int-fsensor', label: '脚温度传感器', result: 'wait' },
                { id: 'int-bsettemp', label: '身体设置温度(0或25~36°C)', result: 'wait' },
                { id: 'int-fsettemp', label: '脚设置温度(0或25~40°C)', result: 'wait' },
                { id: 'int-runstate', label: '运行状态(开+关)', result: 'wait' },
                { id: 'int-runtime', label: '设置运行时间(1~12h)', result: 'wait' },
                { id: 'int-boffcurr', label: '身体关电流', result: 'wait' },
                { id: 'int-boncurr', label: '身体加热电流', result: 'wait' },
                { id: 'int-foffcurr', label: '脚关电流', result: 'wait' },
                { id: 'int-foncurr', label: '脚加热电流', result: 'wait' },
            ]
        };
        // 外部模式(测试遥控)追踪状态
        let extTest = { onoffOk: false, lastPower: null, timeOk: false, bodyOk: false, footOk: false, bodyHas0: false, bodyHas36: false, footHas0: false, footHas40: false };
        // 内部模式(测试加热)追踪状态
        let intTest = {
            devIdOk: false, faultOk: false, bodySensorOk: false, footSensorOk: false,
            bodySetTempOk: false, footSetTempOk: false,
            runStateSeenOn: false, runStateSeenOff: false,
            runTimeOk: false, bodyOffCurrOk: false, bodyOnCurrOk: false, footOffCurrOk: false, footOnCurrOk: false
        };
                ...
                case MSGTYPE.WORK:
                    if (msg.msgOp === MSGOP.SET) {
                        ...
                        if (payload.length === 4) {
                            ...
                            if (hp) {
                                // 身体温度: 0xFF 表示 0 度
                                const bodyTemp = (payload[0] !== 0xff) ? payload[0] : 0;
                                // 脚部温度
        ...
        function msg_ack_report(msgTypeName, payload) {
            if (payload && payload.length === 1) {
                ...
                bleAddLog(
                    `[RECV <<<]`,
                    `${msgTypeName} - RESPONSE | Payload: [${hexStr}]`,
                );
            }
        }
        ...
        function tempMsgHandle(payload) {
            ...
            if (payload.length >= 12) {
                try {
                    // 检查必要的最小长度以避免访问越界
                    const bodyTemp = payload.length >= 2 ?
                        parseFloat((payload[1] + payload[0] / 10).toFixed(1)) : 0.0;
                    const footTemp = payload.length >= 4 ?
                        parseFloat((payload[3] + payload[2] / 10).toFixed(1)) : 0.0;
                    ...
                    // 目标原始温度 4-7
                    const bodyRawTemp = payload.length >= 6 ?
                        parseFloat((payload[5] + payload[4] / 10).toFixed(1)) : 0.0;
                    const footRawTemp = payload.length >= 8 ?
                        parseFloat((payload[7] + payload[6] / 10).toFixed(1)) : 0.0;
                    ...
                    // 目标温度 8-11
                    const bodyTargetTemp = payload.length >= 10 ?
                        parseFloat((payload[9] + payload[8] / 10).toFixed(1)) : 0.0;
                    const footTargetTemp = payload.length >= 12 ?
                        parseFloat((payload[11] + payload[10] / 10).toFixed(1)) : 0.0;
                    ...
                    // 电流数据 21-24
                    let bodyCurrent = 0.0, footCurrent = 0.0;
                    let bodyVoltage = 24.0, footVoltage = 24.0;
                    ...
                    if (payload.length >= 23) {
                        ...
                        if (bodyCurrent < 0.10) bodyCurrent = 0;
                    }
                    if (payload.length >= 25) {
                        ...
                        if (footCurrent < 0.10) footCurrent = 0;
                    }
                    ...
                    if (payload.length >= 29) {
                        bodyCloseTimerCnt = (parseInt(payload[28]) << 8) | parseInt(payload[27]);
                    ...
                    if (payload.length >= 33) {
                        footCloseTimerCnt = (parseInt(payload[32]) << 8) | parseInt(payload[31]);
                    ...
                    if (payload.length >= 35) {
                        bodyHeatUpTimerCnt = (parseInt(payload[34]) << 8) | parseInt(payload[33]);
                    ...
                    if (payload.length >= 37) {
                        footHeatUpTimerCnt = (parseInt(payload[36]) << 8) | parseInt(payload[35]);
                    ...
                    if (payload.length >= 49) {
                        ...
                        if (el) el.textContent = heatModelNames[payload[48]] || `未知(${payload[48]})`;
                    }
                    if (payload.length >= 50) {
                        const el = document.getElementById('footHeatModel');
                        if (el) el.textContent = heatModelNames[payload[49]] || `未知(${payload[49]})`;
                    }
                ...
                // 数据长度不足且不是2字节的短包
                bleAddLog('[警告]', `温度消息数据长度不足: ${payload.length} 字节，期望2字节或至少12字节`);
            }
        }
        ...
        function onSystemInfoReceived(payload, len) {
            ...
            for (let i = 0; i < 12; i++) {
                ...
                idHexArray.push(
                    ("0" + payload[i].toString(16)).slice(-2).toUpperCase(),
                );
            }
            ...
            if (len > 15) {
                ...
                const bodyTemp = parseFloat(
                    (payload[20] + payload[19] / 10).toFixed(1),
                );
                const footTemp = parseFloat(
                    (payload[22] + payload[21] / 10).toFixed(1),
                );

                const trTempBody = parseFloat(
                    (payload[24] + payload[23] / 10).toFixed(1),
                );
                const trTempFoot = parseFloat(
                    (payload[26] + payload[25] / 10).toFixed(1),
                );
        ...
        // POWER消息组装函数
        function assemblePowerMessage(msgOp, powerState, bodyTemp, footTemp, totalDuration) {
            let payload = [];
            // let operationDesc = "";
            switch (msgOp) {
                case MSGOP.GET:
        ...
        function updateSysStatusDisplay(stateValue) {
            ...
            // 清除旧的状态类
            element.classList.remove('status-running', 'status-paused', 'status-off', 'status-error', 'status-non');
        ...
        function updateStatusDisplay(elementId, stateValue, isSysState = false) { // 添加 isSysState 参数
            ...
            if (isSysState) {
                ...
                if (stateValue === HeaterMode.RUNNING) {
                    element.classList.add('status-running');
                } else if ([HeaterMode.TEMP_SUSPENDED, HeaterMode.TIM_SUSPENDED].includes(stateValue)) {
        ...
        function updateThresholdHeating() {
            ...
            // 获取输入值
            const bodyThresholdInput = document.getElementById('bodyThresholdInput').value;
            const footThresholdInput = document.getElementById('footThresholdInput').value;

            bleAddLog(`[信息]`, `更新阀值, 身体阀值温度: ${bodyThresholdInput}/10°C, 脚部阀值温度: ${footThresholdInput}/10°C`);

            const payload = [
                bodyThresholdInput & 0xff,
                footThresholdInput & 0xff,
            ];
        ...
        function updateTempHeating() {
            ...
            bleAddLog(`[信息]`, `直接设置开始加热 - 运行时间: ${runtimeHours}小时, 身体温度: ${bodyTemp}°C, 脚部温度: ${footTemp}°C`);
        ...
        function startHeating() {
            ...
            bleAddLog(`[信息]`, `开始加热 - 运行时间: ${runtimeHours}小时, 身体温度: ${bodyTemp}°C, 脚部温度: ${footTemp}°C`);
        ...
        function updateHeatingButtonStates() {
            ...
            if (!element) {
                ...
                isRunning = element.textContent.trim() === "运行中" || element.classList.contains('status-running');
            }
        ...
        // 更新电流显示的函数
        function updateCurrentDisplay(bodyCurrent, footCurrent, bodyVoltage = 24.0, footVoltage = 24.0) {
        ...
        function resetAllTests() {
            // 重置外部测试状态
            extTest = { onoffOk: false, lastPower: null, timeOk: false, bodyOk: false, footOk: false, bodyHas0: false, bodyHas36: false, footHas0: false, footHas40: false };
            // 重置内部测试状态
            intTest = {
                devIdOk: false, faultOk: false, bodySensorOk: false, footSensorOk: false,
                bodySetTempOk: false, footSetTempOk: false,
            ...
            for (const key of Object.keys(TEST_DEFS)) {
                TEST_DEFS[key].forEach(item => { item.result = 'wait'; });
            }
        }
            ...
            if (testActive && isMqttConnected && mqttClient) {
                ...
                if (prevDefs && prevDefs.some(item => item.result !== 'wait')) {
                    ...
                    const jsonData = {
                        ...
                        ...(prevMode === 'internal' ? { deviceId: devId || '未知', deviceVer: devVer || '未知' } : {}),
                        items: prevDefs.map(item => ({ label: item.label, result: item.result }))
                    };
                    ...
                    bleAddLog('[MQTT]', `发送上次测试结果 → Topic: ${topic}, Payload: ${payload}`);
                }
            }
            testActive = true;
            resetAllTests();
            currentTestMode = document.getElementById('rockerToggle')?.checked ? 'internal' : 'external';
            renderTestPanel();
            bleAddLog('info', `开始测试 → 模式: ${currentTestMode === 'external' ? '测试遥控' : '测试加热'}`);
            // 测试加热模式：先获取一次设备ID
        ...
        // 保存测试：先执行开始测试（使用保存Topic），再清空条码
        function saveTest() {
            startTest(true);
            const barcodeEl = document.getElementById('barcodeInput');
            if (barcodeEl) { barcodeEl.value = ''; updateTestBtnState(); }
        }
        ...
        function updateIntTest(key, value) {
            ...
            if (key === 'devIdOk' && value) {
                defs.find(d => d.id === 'int-devid').result = 'ok';
            }
            // 2. 故障代码：0 为 PASS
            if (key === 'faultCode') {
                intTest.faultOk = true;
                defs.find(d => d.id === 'int-fault').result = value === 0 ? '✅ PASS' : `❌ 0x${value.toString(16).toUpperCase()}`;
            }
            // 3. 身体温度传感器：15~30°C 为 PASS（可配置）
            if (key === 'bodySensor') {
                if (value >= currRange.bodySensorMin && value <= currRange.bodySensorMax) intTest.bodySensorOk = true;
                defs.find(d => d.id === 'int-bsensor').result = intTest.bodySensorOk ? '✅ PASS' : `${value.toFixed(1)}°C (需${currRange.bodySensorMin}~${currRange.bodySensorMax}°C)`;
            }
            // 4. 脚温度传感器：15~30°C 为 PASS（可配置）
            if (key === 'footSensor') {
                if (value >= currRange.footSensorMin && value <= currRange.footSensorMax) intTest.footSensorOk = true;
                defs.find(d => d.id === 'int-fsensor').result = intTest.footSensorOk ? '✅ PASS' : `${value.toFixed(1)}°C (需${currRange.footSensorMin}~${currRange.footSensorMax}°C)`;
            }
            // 5. 身体设置温度：0 或 25~36°C 为 PASS
            if (key === 'bodySetTemp') {
                if (value === 0 || (value >= 25 && value <= 36)) intTest.bodySetTempOk = true;
                defs.find(d => d.id === 'int-bsettemp').result = intTest.bodySetTempOk ? '✅ PASS' : `${value.toFixed(1)}°C`;
            }
            // 6. 脚设置温度：0 或 25~40°C 为 PASS
            if (key === 'footSetTemp') {
                if (value === 0 || (value >= 25 && value <= 40)) intTest.footSetTempOk = true;
                defs.find(d => d.id === 'int-fsettemp').result = intTest.footSetTempOk ? '✅ PASS' : `${value.toFixed(1)}°C`;
            }
            // 7. 运行状态：开+关都出现为 PASS
            if (key === 'runState') {
                ...
                defs.find(d => d.id === 'int-runstate').result = ok ? '✅ PASS' : (value === SysState.ON ? '运行中' : '已关闭');
            }
            // 8. 设置运行时间：1~12h 为 PASS
            if (key === 'runTime') {
                if (value >= 1 && value <= 12) intTest.runTimeOk = true;
                defs.find(d => d.id === 'int-runtime').result = intTest.runTimeOk ? '✅ PASS' : `${value}h`;
            }
            // 9/10. 身体电流检测（使用可配置范围）
            if (key === 'bodyCurrCheck') {
                const raw = value.raw, state = value.state;
                if (state === HeaterMode.OFF && raw >= currRange.bodyOffMin && raw <= currRange.bodyOffMax) intTest.bodyOffCurrOk = true;
                if (state === HeaterMode.RUNNING && raw >= currRange.bodyOnMin && raw <= currRange.bodyOnMax) intTest.bodyOnCurrOk = true;
                defs.find(d => d.id === 'int-boffcurr').result = intTest.bodyOffCurrOk ? '✅ PASS' : `ADC:${raw} (需${currRange.bodyOffMin}~${currRange.bodyOffMax})`;
                defs.find(d => d.id === 'int-boncurr').result = intTest.bodyOnCurrOk ? '✅ PASS' : `ADC:${raw} (需${currRange.bodyOnMin}~${currRange.bodyOnMax})`;
            }
            // 11/12. 脚电流检测（使用可配置范围）
            if (key === 'footCurrCheck') {
                const raw = value.raw, state = value.state;
                if (state === HeaterMode.OFF && raw >= currRange.footOffMin && raw <= currRange.footOffMax) intTest.footOffCurrOk = true;
                if (state === HeaterMode.RUNNING && raw >= currRange.footOnMin && raw <= currRange.footOnMax) intTest.footOnCurrOk = true;
                defs.find(d => d.id === 'int-foffcurr').result = intTest.footOffCurrOk ? '✅ PASS' : `ADC:${raw} (需${currRange.footOffMin}~${currRange.footOffMax})`;
                defs.find(d => d.id === 'int-foncurr').result = intTest.footOnCurrOk ? '✅ PASS' : `ADC:${raw} (需${currRange.footOnMin}~${currRange.footOnMax})`;
            }
            renderTestPanel();
        }
        ...
        function handleKeyboardShortcut(event) {
            ...
            if (handled) {
                ...
                hp._scheduleUI();
                hp._sendToExternal();
                bleAddLog('info', `快捷键 ${event.shiftKey ? 'Shift+' : ''}${key} → 电源:${hp._state.isOn ? 'ON' : 'OFF'} 时间:${hp._state.main.value}h 身体:${hp._state.left.value}°C 脚部:${hp._state.right.value}°C`);
            }
        }
        ...
        // ========== 范围配置（默认值，可密码修改） ==========
        const currRange = {
            bodyOffMin: 0, bodyOffMax: 80,
            bodyOnMin: 1500, bodyOnMax: 1800,
            footOffMin: 0, footOffMax: 80,
            footOnMin: 1500, footOnMax: 1800,
            bodySensorMin: 15, bodySensorMax: 30,
            footSensorMin: 15, footSensorMax: 30
        };
        const CURR_SETTINGS_PWD = '1234';

        function showCurrSettings() {
            const pwd = prompt('请输入设置密码：');
            if (pwd !== CURR_SETTINGS_PWD) { alert('密码错误！'); return; }
            const msg = `当前电流范围设置：\n\n` +
                `身体关电流范围: ${currRange.bodyOffMin} ~ ${currRange.bodyOffMax}  (最大可设 0~200)\n` +
                `身体加热电流范围: ${currRange.bodyOnMin} ~ ${currRange.bodyOnMax}  (最大可设 1200~3200)\n` +
                `脚关电流范围: ${currRange.footOffMin} ~ ${currRange.footOffMax}  (最大可设 0~200)\n` +
                `脚加热电流范围: ${currRange.footOnMin} ~ ${currRange.footOnMax}  (最大可设 1200~3200)\n\n` +
            ...
            if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
                if (parts[0] < 0 || parts[1] > 200) { alert('身体关电流范围超出最大限制 0~200！'); return; }
                currRange.bodyOffMin = parts[0]; currRange.bodyOffMax = parts[1];
            }
            const input2 = prompt(`身体加热电流范围 (最大 1200~3200):`, `${currRange.bodyOnMin}-${currRange.bodyOnMax}`);
            if (input2) {
                const p2 = input2.split('-').map(Number);
                if (p2.length === 2 && !isNaN(p2[0]) && !isNaN(p2[1])) {
                    if (p2[0] < 1200 || p2[1] > 3200) { alert('身体加热电流范围超出最大限制 1200~3200！'); return; }
                    currRange.bodyOnMin = p2[0]; currRange.bodyOnMax = p2[1];
                }
            }
            const input3 = prompt(`脚关电流范围 (最大 0~200):`, `${currRange.footOffMin}-${currRange.footOffMax}`);
            if (input3) {
                const p3 = input3.split('-').map(Number);
                if (p3.length === 2 && !isNaN(p3[0]) && !isNaN(p3[1])) {
                    if (p3[0] < 0 || p3[1] > 200) { alert('脚关电流范围超出最大限制 0~200！'); return; }
                    currRange.footOffMin = p3[0]; currRange.footOffMax = p3[1];
                }
            }
            const input4 = prompt(`脚加热电流范围 (最大 1200~3200):`, `${currRange.footOnMin}-${currRange.footOnMax}`);
            if (input4) {
                const p4 = input4.split('-').map(Number);
                if (p4.length === 2 && !isNaN(p4[0]) && !isNaN(p4[1])) {
                    if (p4[0] < 1200 || p4[1] > 3200) { alert('脚加热电流范围超出最大限制 1200~3200！'); return; }
                    currRange.footOnMin = p4[0]; currRange.footOnMax = p4[1];
                }
            }
            const input5 = prompt(`身体温度传感器范围 (最大 0~50°C):`, `${currRange.bodySensorMin}-${currRange.bodySensorMax}`);
            if (input5) {
                const p5 = input5.split('-').map(Number);
                if (p5.length === 2 && !isNaN(p5[0]) && !isNaN(p5[1])) {
                    if (p5[0] < 0 || p5[1] > 50) { alert('身体温度传感器范围超出最大限制 0~50！'); return; }
                    currRange.bodySensorMin = p5[0]; currRange.bodySensorMax = p5[1];
                }
            }
            const input6 = prompt(`脚温度传感器范围 (最大 0~50°C):`, `${currRange.footSensorMin}-${currRange.footSensorMax}`);
            if (input6) {
                const p6 = input6.split('-').map(Number);
                if (p6.length === 2 && !isNaN(p6[0]) && !isNaN(p6[1])) {
                    if (p6[0] < 0 || p6[1] > 50) { alert('脚温度传感器范围超出最大限制 0~50！'); return; }
                    currRange.footSensorMin = p6[0]; currRange.footSensorMax = p6[1];
                }
            }
            bleAddLog('info', `范围已更新: 电流-身体关:${currRange.bodyOffMin}-${currRange.bodyOffMax} 身体开:${currRange.bodyOnMin}-${currRange.bodyOnMax} 脚关:${currRange.footOffMin}-${currRange.footOffMax} 脚开:${currRange.footOnMin}-${currRange.footOnMax} 温度-身体:${currRange.bodySensorMin}-${currRange.bodySensorMax}°C 脚:${currRange.footSensorMin}-${currRange.footSensorMax}°C`);
        }
        ...
        async function downloadData() {
            // const url = 'https://red.unito.top/unito/stoov/heat-bed/test-data/datat-dl';
            // 使用相对路径，请求会自动发给当前域名（www.unito.top）
            const url = '/unito/stoov/heat-bed/test-data/datat-dl';
            try {
                bleAddLog('info', '正在下载数据...');
        ...
        function bindEvents() {
            ...
            document.getElementById('barcodeInput').addEventListener('input', updateTestBtnState);
            // 初始化加热面板事件监听
            ...
            timerIntervalInput.addEventListener('keydown', async function (event) {
                if (event.key === 'Enter') {
                    ...
                    console.log(`Input Value: ${inputValue}, Displayed Value: ${currentDisplayValue}`); // 用于调试
