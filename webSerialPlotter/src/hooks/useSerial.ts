...
export function useSerial(): UseSerial {
  const [state, setState] = useState<SerialState>({
    isSupported:
      typeof navigator !== "undefined" && !!(navigator as Navigator).serial,
    isConnecting: false,
    isConnected: false,
    port: null,
    readerLocked: false,
  ...
  const connect = useCallback(
    async (baudRate: number) => {
      if (!state.isSupported) {
        setState((s) => ({
          ...s,
          error: "Web Serial not supported in this browser.",
        }));
        return;
      }
  ...
  const write = useCallback(async (data: string) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to write to serial port";
      setState((s) => ({ ...s, error: message }));
      throw err;
    }
  }, []);
